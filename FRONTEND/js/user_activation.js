...
const checkUserActivation = async (user_id, activation_id) =>{
    await fetch("http://localhost:8000/activation", {
            method: "POST",
            mode: "cors",
            headers:{"Content-Type": "application/json", 
        "Access-Control-Allow-Origin": "*"},
        body: JSON.stringify({id: user_id, activationLink: activation_id})
        }).then((data)=>{
            response = data.json();
...
document.getElementById('submit').addEventListener('click', async function (){
    ...
    if(password.trim().length < 8){
        ...
        if(await userActivation(response.userId, password)){
...
const userActivation = async (user_id, password) =>{
    var response = null;
    await fetch("http://localhost:8000/registerPassword", {
            method: "POST",
            mode: "cors",
            headers:{"Content-Type": "application/json", 
        "Access-Control-Allow-Origin": "*"},
